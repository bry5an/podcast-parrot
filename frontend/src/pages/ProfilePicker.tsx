        ...
        <div style={styles.selectView}>
          ...
          <div style={styles.tileRow}>
            ...
            <div onClick={startCreate} style={styles.tile}>
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ font: '600 16px/1.2 IBM Plex Sans', color: 'rgb(var(--fg-rgb) / .55)' }}>Add learner</div>
                <div style={{ font: '400 11.5px/1.4 IBM Plex Sans', color: 'rgb(var(--fg-rgb) / .4)', marginTop: 7 }}>
                  New profile
                </div>
              </div>
            </div>
              ...
              {PALETTE.map((s, i) => (
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    background: s.art,
                    border: `2px solid ${i === paletteIndex ? 'var(--fg)' : 'transparent'}`,
                    padding: 0,
                  }}
                />
              ))}
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      textAlign: 'left',
                      background: sel ? 'oklch(0.55 0.055 195 / 0.1)' : 'rgb(var(--bg-surface-rgb) / .55)',
                      border: `1.5px solid ${sel ? 'oklch(0.55 0.055 195)' : 'rgb(var(--fg-rgb) / .12)'}`,
                    }}
                  >
          ...
          {learningLanguage === 'ja' && (
            <label style={styles.furiganaRow}>
              <div>
                ...
                <div style={{ font: '400 11px/1.3 IBM Plex Sans', color: 'rgb(var(--fg-rgb) / .5)', marginTop: 3 }}>
              ...
              <button
                ...
                style={{
                  ...
                  padding: 3,
                  display: 'flex',
                  background: showFurigana ? 'oklch(0.55 0.055 195)' : 'rgb(var(--fg-rgb) / .2)',
                  justifyContent: showFurigana ? 'flex-end' : 'flex-start',
                }}
              >
                <span style={{ width: 20, height: 20, borderRadius: '50%', background: 'var(--bg-surface)', boxShadow: '0 1px 3px rgba(0,0,0,.25)' }} />
              </button>
            </label>
          )}
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
            ...
            <button
              ...
              style={{
                ...
                background: name.trim() ? 'var(--fg)' : 'rgb(var(--fg-rgb) / .14)',
                color: name.trim() ? 'var(--bg-page)' : 'rgb(var(--fg-rgb) / .4)',
              }}
            >
...
const styles: Record<string, React.CSSProperties> = {
  ...
  addAvatar: { width: 118, height: 118, borderRadius: 26, border: '1.5px dashed rgb(var(--fg-rgb) / .25)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgb(var(--fg-rgb) / .4)' },
  ...
  label: { font: '600 11px/1 IBM Plex Sans', color: 'rgb(var(--fg-rgb) / .55)', display: 'block', marginBottom: 7 },
  input: { width: '100%', height: 42, padding: '0 14px', borderRadius: 11, border: '1px solid rgb(var(--fg-rgb) / .16)', background: 'var(--bg-surface)', font: '400 14px/1 IBM Plex Sans', color: 'var(--fg)', outline: 'none' },
  furiganaRow: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 15px', borderRadius: 12, background: 'rgb(var(--bg-surface-rgb) / .6)', border: '1px solid rgb(var(--fg-rgb) / .1)', cursor: 'pointer' },
  cancelBtn: { height: 44, padding: '0 20px', borderRadius: 22, border: '1px solid rgb(var(--fg-rgb) / .14)', background: 'var(--bg-surface)', font: '600 13px/1 IBM Plex Sans', color: 'rgb(var(--fg-rgb) / .6)', cursor: 'pointer' },
};
